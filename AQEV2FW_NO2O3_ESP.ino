...
#define EEPROM_O3_SENSITIVITY     (EEPROM_NO2_CAL_OFFSET - 4)     // float value, 4-bytes, the sensitivity from the sticker
...
#define EEPROM_USER_LATITUDE_DEG  (EEPROM_O3_ZERO_NEGATIVE_RESULTS - 4) // float value, 4-bytes, user specified latitude in degrees
...
#define EEPROM_BACKUP_O3_SENSITIVITY     (EEPROM_BACKUP_NO2_CAL_OFFSET + 4)
...
const char cmd_string_o3_sen[] PROGMEM      = "o3_sen     ";
const char cmd_string_o3_slope[] PROGMEM    = "o3_slope   ";
const char cmd_string_o3_off[] PROGMEM      = "o3_off     ";
  ...
  if(first || ((current_millis - previous_display_update) >= display_update_interval)){
    ...
    default:
      ...
      if(init_no2_we_adc_ok && init_no2_aux_adc_ok){
        if(no2_we_ready && no2_aux_ready || (sample_buffer_idx > 0)){
      ...
      if(init_o3_afe_ok && init_o3_adc_ok){
        if(o3_ready || (sample_buffer_idx > 0)){
          updateLCD(o3_ppb, 13, 1, 3, false);
        }
        else{
...
void initializeHardware(void) {
  ...
  Serial.println(F(" |       NO2 / O3 Sensor Suite        |"));  
...
void initializeNewConfigSettings(void){
  ...
  if(val != 1){
    ...
    configInject("sampling 5, 160, 60\r");
...
void restore(char * arg) {
  ...
  if (strncmp(arg, "defaults", 8) == 0) {
    ...
    eeprom_write_block("aqev2_no2_o3_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
  }  
...
boolean publishHeartbeat(){
  ...
  "\"converted-value\":%d,"
  "\"firmware-version\":\"%s\","
  "\"publishes\":[\"no2\",\"o3\",\"temperature\",\"humidity\"],"
  "\"counter\":%lu"
...
boolean publishNO2(){
  clearTempBuffers();
  uint16_t num_samples = no2_we_ready && no2_aux_ready ? sample_buffer_depth : sample_buffer_idx;
  float converted_value = 0.0f, compensated_value = 0.0f;    
  float no2_we_moving_average = calculateAverage(&(sample_buffer[NO2_WE_SAMPLE_BUFFER][0]), num_samples);
  float no2_aux_moving_average = calculateAverage(&(sample_buffer[NO2_AUX_SAMPLE_BUFFER][0]), num_samples);
...
boolean publishO3(){
  clearTempBuffers();  
  uint16_t num_samples = o3_ready ? sample_buffer_depth : sample_buffer_idx;
  float converted_value = 0.0f, compensated_value = 0.0f;   
  float o3_moving_average = calculateAverage(&(sample_buffer[O3_SAMPLE_BUFFER][0]), num_samples);
...
void printCsvDataLine(){
  ...
  num_samples = no2_we_ready && no2_aux_ready ? sample_buffer_depth : sample_buffer_idx;
  if(no2_we_ready && no2_aux_ready && ((no2_we_ready && no2_aux_ready) || (sample_buffer_idx > 0))){
    float converted_value = 0.0f, compensated_value = 0.0f;    
    no2_we_moving_average = calculateAverage(&(sample_buffer[NO2_WE_SAMPLE_BUFFER][0]), num_samples);
    no2_aux_moving_average = calculateAverage(&(sample_buffer[NO2_AUX_SAMPLE_BUFFER][0]), num_samples);
    no2_convert_from_volts_to_ppb(no2_we_moving_average, no2_aux_moving_average, &converted_value, &compensated_value);
    no2_ppb = compensated_value;      
  ...
  if(init_o3_afe_ok && init_o3_adc_ok && (o3_ready || (sample_buffer_idx > 0))){
    float converted_value = 0.0f, compensated_value = 0.0f;   
